...
_KEYWORD_MAP = [
    ...
    # Monthly closing MUST come before year-end closing (more specific "clôture mensuelle" before generic "clôture")
    (TaskType.MONTHLY_CLOSING, [r"\bclôture\s+mensuelle\b", r"\bmåneds\w*\s*avslut\w*\b",
                                   r"\bmonthly\s*clos", r"\bperiod\w*\s*clos",
                                   r"\brégularisation\b", r"\baccrual\b",
                                   r"\bMonatsabschluss\w*\b", r"\bcierre\s+mensual\b",
                                   r"\bmånads\w*\s*avslut\w*\b"]),
    # Analyze ledger — must come before year-end closing
    (TaskType.ANALYZE_LEDGER, [r"\banalyser?\w*\b.*\b(hovudbok|hovedbok|ledger|libro\s*mayor|grand\s*livre|Hauptbuch)\b",
                                  r"\b(hovudbok|hovedbok|libro\s*mayor)\w*\b.*\banalyser?\w*\b",
                                  r"\banalice\b.*\b(libro|mayor|costos|cuentas)\b",
                                  r"\b(kostnad|cost|costo|coût|Kosten)\w*\b.*\b(auk|increase|øk|augment|stig)\w*\b",
                                  r"\b(identifi|finn|find|trouvez|encuentre)\w*\b.*\b(kostnad|cost|costo|expense|utgift|konto|account|cuenta|compte)\w*\b"]),
    ...
    (TaskType.REGISTER_PAYMENT, [r"\b(registrer|register)\b.*\b(betaling|innbetaling|payment|pago)\b",
                                  ...
                                  r"\b(überfällig|overdue|forfalt|forfall|vencid|impagad|en\s+retard)\w*\b.*\b(rechnung|faktura|invoice|factura)\b",
                                  r"\b(rechnung|faktura|invoice|factura)\w*\b.*\b(überfällig|overdue|forfalt|vencid)\w*\b",
                                  r"\b(buchen|book)\w*\b.*\b(zahlung|payment|betaling)\b"]),
